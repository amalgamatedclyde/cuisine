...
f <- function(c){colMeans(m[rownames(m)==c, ])}
z <- sapply(unique.cuisines,  f)
z1 <- t(z)
knn_models <- list(list())
set.seed(314159)
for(item in unique.cuisines)
  item = "indian"
  cuisine <- readRDS("cuisine.RDS")
  m <- readRDS("m.RDS")
  # recipes <- m[rownames(m)==item,]
  # indian <- indian[sample(nrow(indian),size=600,replace=FALSE),]
  # not_indian <- m[rownames(m)!="italian",]
  # not_indian <- not_indian[sample(nrow(not_indian),size=400,replace=FALSE),]
  rownames(m[rownames(m)!=item,]) <- rep("not", nrow(m))
  # test_set <- rbind(indian, not_indian)
  y <- as.factor(rownames(m))
  knn_models <- c(knn_models, knn(m, w, cl=y, k=11, prob = T))
