...
apply(ingredient.weight.matrix, 1, function(x) x/max(x)*idf)
...
for(num in 5000:5001){test.weight.matrix[num,] <- is.ingredient.in.recipe(num)}#takes_forever

sapply(1:nrow(test.df), is.ingredient.in.recipe)
#sapply(unique.ingredients, function(x, num) ingredient %in% unlist(test.df[num,]$ingredients)) #working on thistes 
...
find.match <- function(row){apply(ingredient.weight.matrix, 1, function(x) sum(x*row))}
